...
public class LoadTimeSheet {
 ...
 organizing data by week.
     ...
     * @param employee The list where the loaded employee time logs will be stored
     */
    public static void loadTimeSheet(String targetEmployeeId, String attendanceRecordPath, Employee employee) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(attendanceRecordPath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                TimeLogs entry = new TimeLogs(employeeId, date, logIn, logOut, hoursWorked, overTime, weekNumber);

                // Add the entry to our timesheet array list
                employee.addTimeLog(entry);
            }
        } catch (IOException e) {
